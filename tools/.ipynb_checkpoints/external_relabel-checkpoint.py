...
def train(cfg, local_rank, distributed, logger):
    ...
    for iteration, (images, targets, _) in tqdm(enumerate(train_data_loader)):
        ...
        for t, pairwise_logit, logits in zip(targets, pairwise_logits, relation_logits):
            cur_data = t.get_field("cur_data")
            cur_data = modify_logits(cur_data, logits)
            # cur_data = modify_pairs_logits(cur_data, pairwise_logit)
            cur_data = modify_rel_logits(cur_data, logits)
...
# def modify_pairs_logits(cur_data, rel_logits):
#     # just append the logits
#     # if rel_logits != 'N':
#         # cur_data["label_logits"] = rel_logits.cpu().numpy()
#     return cur_data

def modify_rel_logits(cur_data, rel_logits):
    # just append the logits
    cur_data["pred_rel_logits"] = rel_logits.cpu().numpy()
    return cur_data
...
def main():
    ...
    save_path = os.path.join(output_dir, cfg.Confusion_Name)
