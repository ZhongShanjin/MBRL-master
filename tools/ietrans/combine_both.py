...
def complete_relatinos(data):
    ...
    for poss_rls, logits, pair in zip(possible_rels, rel_logits, pairs):
        ...
        if idx2pred.get(poss_rls[max_id] - 1, 'none') == 'flying in':
            print('f')
    ...
    data['ori_relations'] = deepcopy(data['relations'])
    data['grain_size'] = np.ones(len(data["relations"]))
    assert len(data["grain_size"]) == len(data["relations"]) == len(data["ori_relations"])
    del data["pairs"]
    return data
...
def merge_ex_and_in(ex_data, in_data):
    ...
    in_ori_rels = in_data["ori_relations"]
    in_pairs = in_rels[:, :2]
    ...
    final_ori_rels = np.concatenate([in_ori_rels, ex_rels[unintersect_rels]], 0)
    in_data["relations"] = final_rels
    in_data["ori_relations"] = final_ori_rels
    in_data["grain_size"] = np.concatenate([in_data["grain_size"], ex_data["grain_size"][unintersect_rels]], 0)
    assert  len(in_data["grain_size"]) == len(in_data["relations"]) == len(in_data["ori_relations"])
    return in_data
...
pickle.dump(to_save_data, open("em_EEE.pk", "wb"))
