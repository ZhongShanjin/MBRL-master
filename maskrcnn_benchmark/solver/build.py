...
def make_optimizer(cfg, model, logger, slow_heads=None, slow_ratio=5.0, rl_factor=1.0):
    ...
    optimizer = torch.optim.SGD(params, lr=cfg.SOLVER.BASE_LR, momentum=cfg.SOLVER.MOMENTUM) #0.01 0.9
    return optimizer
...
def make_lr_scheduler(cfg, optimizer, logger=None):
    if cfg.SOLVER.SCHEDULE.TYPE == "WarmupMultiStepLR":
        ...
        return WarmupReduceLROnPlateau(
            optimizer,
            cfg.SOLVER.SCHEDULE.FACTOR, #0.1
            ...
            patience=cfg.SOLVER.SCHEDULE.PATIENCE, #2
            threshold=cfg.SOLVER.SCHEDULE.THRESHOLD, #0.001
            cooldown=cfg.SOLVER.SCHEDULE.COOLDOWN, #0
            logger=logger,
        )
