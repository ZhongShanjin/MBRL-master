...
class TransformerPredictor(nn.Module):
    def __init__(self, config, in_channels):
        ...
        if 'GQA' in str(config.DATASETS.TRAIN):  # GQA
            obj_classes, rel_classes, att_classes = statistics['obj_classes'], statistics['rel_classes'], None
        else:
...
class MotifPredictor(nn.Module):
    def __init__(self, config, in_channels):
        ...
        self.attribute_on = config.MODEL.ATTRIBUTE_ON  # F
        self.num_obj_cls = config.MODEL.ROI_BOX_HEAD.NUM_CLASSES  # 151
        self.num_att_cls = config.MODEL.ROI_ATTRIBUTE_HEAD.NUM_ATTRIBUTES  # 201
        self.num_rel_cls = config.MODEL.ROI_RELATION_HEAD.NUM_CLASSES  # 51

        assert in_channels is not None  # 4096
        num_inputs = in_channels  # 4096
        self.use_vision = config.MODEL.ROI_RELATION_HEAD.PREDICT_USE_VISION  # T
        # train_use_bias is to use bias in the training
        self.train_use_bias = config.MODEL.ROI_RELATION_HEAD.TRAIN_USE_BIAS  # F
        # predict_use_bias is to use bias in the inference
        self.predict_use_bias = config.MODEL.ROI_RELATION_HEAD.PREDICT_USE_BIAS  # F
        # load class dict
        statistics = get_dataset_statistics(config)
        if 'GQA' in str(config.DATASETS.TRAIN): #GQA
            obj_classes, rel_classes, att_classes = statistics['obj_classes'], statistics['rel_classes'], None
        ...
        self.pooling_dim = config.MODEL.ROI_RELATION_HEAD.CONTEXT_POOLING_DIM  # 409
        ...
        self.rel_compress = nn.Linear(self.pooling_dim, self.num_rel_cls, bias=True)  # (4096,51)
