...
HEAD_IDS = torch.tensor([8, 20, 22, 29, 30, 31, 48])
BODY_IDS = torch.tensor([1, 5, 6, 7, 9, 11, 16, 19, 21, 23, 25, 33, 35, 38, 40, 41, 43, 46, 47, 49, 50])
TAIL_IDS = torch.tensor([2, 3, 4, 10, 12, 13, 14, 15, 17, 18, 24, 26, 27, 28, 32, 34, 36, 37, 39, 42, 44, 45])
...
class RelationLossComputation(object):
    ...
    def __call__(self, proposals, rel_labels, relation_logits, refine_logits):
        ...
        loss_relation = self.criterion_loss(relation_logits, rel_labels.long()) #log以10为底
...
class WRelationLossComputation(object):
    ...
    def __init__(
        ...
        elif self.rel_loss == 'cemean':
            self.rel_criterion_loss = CEForSoftLabel('cemean')
        elif self.rel_loss == "ce_rwt":
            self.rel_criterion_loss = ReweightingCE()


    def __call__(self, proposals, rel_labels, relation_logits, refine_logits, ori_rel_labels, grain_size, pos_weight=None):
        """
        Computes the loss for relation triplet.
        This requires that the subsample method has been called beforehand.
        ...
        fg_labels = cat([proposal.get_field("labels") for proposal in proposals], dim=0)
        rel_labels = cat(rel_labels, dim=0)
        loss_relation = self.rel_criterion_loss(relation_logits, rel_labels, torch.cat(ori_rel_labels), torch.cat(grain_size))
...
class CEForSoftLabel(nn.Module):
    ...
    def forward(self, input, target, ori_rel_labels, grain_size, pos_weight=None):
        #cur_data
        final_target = torch.zeros_like(target)
        final_target[torch.arange(0, target.size(0)), target.argmax(1)] = 1.
        #ori_data
        final_ori_target = torch.zeros_like(ori_rel_labels)
        final_ori_target[torch.arange(0, ori_rel_labels.size(0)), ori_rel_labels.argmax(1)] = 1.
        #cal_loss
        # for bi_idxs, bi_logits in enumerate(input):
        #     for current_idxs, current_logits in enumerate(bi_logits):
        x = F.log_softmax(input, 1)

        new_edge_weight = torch.ones(len(grain_size)).to(final_target)
        old_edge_weight = torch.ones(len(grain_size)).to(final_target)
        idx_n = torch.nonzero(torch.squeeze(grain_size >= 0.95)).data.view(-1)
        idx_o = torch.nonzero(torch.squeeze((1 - grain_size) >= 0.95)).data.view(-1)
        idx_bg = torch.nonzero(torch.squeeze((1 - grain_size) == 1)).data.view(-1)
        new_edge_weight[idx_n] = 5
        old_edge_weight[idx_o] = 5
        old_edge_weight[idx_bg] = 1
        
        bi_loss = torch.sum(- x * final_target * grain_size, dim=1) * new_edge_weight + torch.sum(- x * final_ori_target * (1 - grain_size), dim=1) * old_edge_weight
        # bi_loss = torch.sum(- x * final_target * grain_size, dim=1) + torch.sum(- x * final_ori_target * (1 - grain_size), dim=1)
        loss = torch.sum(- x * target, dim=1)
        if self.reduction == 'none':
            return loss
        elif self.reduction == 'sum':
            return torch.sum(loss)
        elif self.reduction == 'cemean':
            return torch.mean(loss)
        elif self.reduction == 'mean':
            # return torch.mean(loss)
            # print('torch.mean(bi_loss) = ', torch.mean(bi_loss))
            return torch.mean(bi_loss)
        else:
...
def make_roi_relation_loss_evaluator(cfg):
    ...
    loss_evaluator = RelationLossComputation(
        cfg.MODEL.ATTRIBUTE_ON, #F
        cfg.MODEL.ROI_ATTRIBUTE_HEAD.NUM_ATTRIBUTES, #201
        cfg.MODEL.ROI_ATTRIBUTE_HEAD.MAX_ATTRIBUTES, #10
        cfg.MODEL.ROI_ATTRIBUTE_HEAD.ATTRIBUTE_BGFG_SAMPLE, #T
        cfg.MODEL.ROI_ATTRIBUTE_HEAD.ATTRIBUTE_BGFG_RATIO, #3
        cfg.MODEL.ROI_RELATION_HEAD.LABEL_SMOOTHING_LOSS, #F
