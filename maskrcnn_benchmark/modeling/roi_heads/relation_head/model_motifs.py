...
class DecoderRNN(nn.Module):
    def __init__(self, config, obj_classes, embed_dim, inputs_dim, hidden_dim, rnn_drop):
        ...
        self.embed_dim = embed_dim #200

        obj_embed_vecs = obj_edge_vectors(['start'] + self.obj_classes, wv_dir=self.cfg.GLOVE_DIR, wv_dim=embed_dim) #维度为类别数量+2
        self.obj_embed = nn.Embedding(len(self.obj_classes)+1, embed_dim)
        with torch.no_grad():
            self.obj_embed.weight.copy_(obj_embed_vecs, non_blocking=True)

        self.hidden_size = hidden_dim #512
        self.inputs_dim = inputs_dim #4936
        self.input_size = self.inputs_dim + self.embed_dim #5136
        self.nms_thresh = self.cfg.TEST.RELATION.LATER_NMS_PREDICTION_THRES #0.5
        self.rnn_drop=rnn_drop #0.2

        self.input_linearity = torch.nn.Linear(self.input_size, 6 * self.hidden_size, bias=True) #(5136, 3072)
        self.state_linearity = torch.nn.Linear(self.hidden_size, 5 * self.hidden_size, bias=True) #(512, 2560)
        self.out_obj = nn.Linear(self.hidden_size, len(self.obj_classes)) #(512,151)
...
class LSTMContext(nn.Module):
    ...
    def __init__(self, config, obj_classes, rel_classes, in_channels):
        ...
        # mode
        if self.cfg.MODEL.ROI_RELATION_HEAD.USE_GT_BOX: #T
            if self.cfg.MODEL.ROI_RELATION_HEAD.USE_GT_OBJECT_LABEL: #T
                self.mode = 'predcls'
            else:
                self.mode = 'sgcls'
        else:
        ...
        self.obj_embed1 = nn.Embedding(self.num_obj_classes, self.embed_dim) #(类数,200)
        self.obj_embed2 = nn.Embedding(self.num_obj_classes, self.embed_dim) #(类数,200)
        with torch.no_grad():
        ...
        self.dropout_rate = self.cfg.MODEL.ROI_RELATION_HEAD.CONTEXT_DROPOUT_RATE #0.2
        self.hidden_dim = self.cfg.MODEL.ROI_RELATION_HEAD.CONTEXT_HIDDEN_DIM #512
        self.nl_obj = self.cfg.MODEL.ROI_RELATION_HEAD.CONTEXT_OBJ_LAYER #1
        self.nl_edge = self.cfg.MODEL.ROI_RELATION_HEAD.CONTEXT_REL_LAYER #1
        assert self.nl_obj > 0 and self.nl_edge > 0

        # TODO Kaihua Tang
        # AlternatingHighwayLSTM is invalid for pytorch 1.0
        self.obj_ctx_rnn = torch.nn.LSTM(
                input_size=self.obj_dim+self.embed_dim + 128, #4424
                hidden_size=self.hidden_dim, #512
                num_layers=self.nl_obj, #1
                dropout=self.dropout_rate if self.nl_obj > 1 else 0, #0.2
                bidirectional=True)
        ...
        self.edge_ctx_rnn = torch.nn.LSTM(
                input_size=self.embed_dim + self.hidden_dim + self.obj_dim, #4808
                hidden_size=self.hidden_dim, #512
                num_layers=self.nl_edge, #1
                dropout=self.dropout_rate if self.nl_edge > 1 else 0, #0.2
                bidirectional=True)
        # map bidirectional hidden states of dimension self.hidden_dim*2 to self.hidden_dim
        self.lin_obj_h = nn.Linear(self.hidden_dim*2, self.hidden_dim) #(1024,512)
        self.lin_edge_h = nn.Linear(self.hidden_dim*2, self.hidden_dim) #(1024,512)
        
        # untreated average features
        self.average_ratio = 0.0005
        self.effect_analysis = config.MODEL.ROI_RELATION_HEAD.CAUSAL.EFFECT_ANALYSIS #T

        if self.effect_analysis:
            self.register_buffer("untreated_dcd_feat", torch.zeros(self.hidden_dim + self.obj_dim + self.embed_dim + 128)) #4936
            self.register_buffer("untreated_obj_feat", torch.zeros(self.obj_dim+self.embed_dim + 128)) #4424
            self.register_buffer("untreated_edg_feat", torch.zeros(self.embed_dim + self.obj_dim)) #4296
