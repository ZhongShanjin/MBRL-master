...
class ROIRelationHead(torch.nn.Module):
    ...
    def __init__(self, cfg, in_channels):
        ...
        self.predictor = make_roi_relation_predictor(cfg, feat_dim) #使用哪个SGG模型
        ...
        self.em_E_step = cfg.EM.MODE == "E" #F
        self.em_SGE_step = cfg.EM.MODE == "SGE" #F
        self.eval_only_acc = cfg.TEST.ONLY_ACC #F

        self.loss_evaluator = make_roi_relation_loss_evaluator(cfg)
        self.samp_processor = make_roi_relation_samp_processor(cfg)

        # parameters
        self.use_union_box = self.cfg.MODEL.ROI_RELATION_HEAD.PREDICT_USE_VISION #T
    ...
    def forward(self, features, proposals, targets=None, logger=None):
            ...
            with torch.no_grad():
                if self.cfg.MODEL.ROI_RELATION_HEAD.USE_GT_BOX:
                    if self.weak_sup and self.training:
                        proposals, rel_labels, rel_pair_idxs, rel_binarys, ori_rel_labels, grain_size = self.wsup_samp_processor.gtbox_relsample(proposals, targets)
                    else:
                        proposals, rel_labels, rel_pair_idxs, rel_binarys = self.samp_processor.gtbox_relsample(proposals, targets)
                else:
                    if self.weak_sup and self.training:
                        proposals, rel_labels, rel_pair_idxs, rel_binarys, ori_rel_labels, grain_size = self.wsup_samp_processor.detect_relsample(proposals, targets)
                    else:
                        proposals, rel_labels, rel_pair_idxs, rel_binarys = self.samp_processor.detect_relsample(proposals, targets)
        elif self.em_E_step or self.em_SGE_step:
        ...
        # for em_E_step
        if self.em_E_step:
            return 'F', relation_logits, {}

        # for test
        if not self.training:
            result = self.post_processor((relation_logits, refine_logits), rel_pair_idxs, proposals, targets)
        ...
        if self.weak_sup and self.training:
            ...
            loss_relation, loss_refine = self.wsup_loss_evaluator(proposals, rel_labels, relation_logits, refine_logits, ori_rel_labels, grain_size, attn_score)
        else:
