...
class WRelationSampling(object):
    ...
    def gtbox_relsample(self, proposals, targets):
        ...
        for img_id, (proposal, target) in enumerate(zip(proposals, targets)):
            ...
            ori_rel_labs = target.get_field("ori_relation_labels")
            grain_size_matrix = target.get_field("grain_size")
            ...
            if tgt_pair_idxs.shape[0] > num_pos_per_img:
                ...
                tgt_pair_idxs = tgt_pair_idxs[perm]
                tgt_rel_labs = tgt_rel_labs[perm]
                ori_rel_labs = ori_rel_labs[perm]
                grain_size_matrix = grain_size_matrix[perm]
            ...
            # TODO  bg pairs
            bg_labels[:, 0] = 1.
            grain_size_labels = torch.zeros((tgt_bg_idxs.shape[0], 1), device=device, dtype=torch.float32)
            img_rel_labels = torch.cat((tgt_rel_labs, bg_labels), dim=0)
            img_ori_rel_labels = torch.cat((ori_rel_labs, bg_labels), dim=0)
            img_grain_size_matrix = torch.cat((grain_size_matrix, grain_size_labels), dim=0)
            # if tgt_soft_labels is not None:
            ...
            rel_idx_pairs.append(img_rel_idxs)
            rel_labels.append(img_rel_labels)
            ori_rel_labels.append(img_ori_rel_labels)
            grain_size.append(img_grain_size_matrix)
        # if len(rel_soft_labels) > 1:
        #     rel_sym_binarys = rel_soft_labels
        return proposals, rel_labels, rel_idx_pairs, rel_sym_binarys, ori_rel_labels, grain_size
    ...
    def detect_relsample(self, proposals, targets):
        ...
        for img_id, (proposal, target) in enumerate(zip(proposals, targets)):
            ...
            img_rel_triplets, img_pair_idxs, binary_rel, img_ori_rel_labels, img_grain_size_matrix = self.motif_rel_fg_bg_sampling(device, tgt_pair_idxs, ious, is_match,
    ...
    def motif_rel_fg_bg_sampling(self, device, tgt_pair_idxs, ious, is_match, rel_possibility, target):
        ...
        for i in range(num_tgt_rels):
            ...
            ori_rel_lab = ori_rel_labs[i]
            grain_size = grain_size_matrix[i]
            # find matching pair in proposals (might be more than one)
            prp_head_idxs = torch.nonzero(is_match[tgt_head_idx]).squeeze(1)
            prp_tail_idxs = torch.nonzero(is_match[tgt_tail_idx]).squeeze(1)
            num_match_head = prp_head_idxs.shape[0]
            ...
            ori_fg_labels = ori_rel_lab.repeat(prp_tail_idxs.shape[0], 1)
            grain_size = grain_size.repeat(prp_tail_idxs.shape[0], 1)
            # fg_rel_i = cat((prp_head_idxs.view(-1, 1), prp_tail_idxs.view(-1, 1), fg_labels), dim=-1).to(torch.float32)
            fg_rel_i = fg_labels
            ori_fg_rel_i = ori_fg_labels
            grain_size_i = grain_size
            ...
            if fg_rel_i.shape[0] > self.num_sample_per_gt_rel:
                ...
                perm = npr.choice(ious_score.shape[0], p=ious_score, size=self.num_sample_per_gt_rel, replace=False)
                fg_rel_i = fg_rel_i[perm]
                ori_fg_rel_i = ori_fg_rel_i[perm]
                grain_size_i = grain_size_i[perm]
                fg_pair_i = fg_pair_i[perm]
            if fg_rel_i.shape[0] > 0:
                fg_pair_idxs.append(fg_pair_i)
                fg_rel_triplets.append(fg_rel_i)
                ori_rel_triplets.append(ori_fg_rel_i)
                grain_size_max.append(grain_size_i)

        # select fg relations
        if len(fg_rel_triplets) == 0:
            fg_rel_triplets = torch.zeros((0, self.num_rel_classes), dtype=torch.float32, device=device)
            ori_rel_triplets = torch.zeros((0, self.num_rel_classes), dtype=torch.float32, device=device)
            grain_size_max = torch.zeros((0, 1), dtype=torch.float32, device=device)
            fg_pair_idxs = torch.zeros((0, 2), dtype=torch.int64, device=device)
        else:
            fg_rel_triplets = cat(fg_rel_triplets, dim=0)
            ori_rel_triplets = cat(ori_rel_triplets, dim=0)
            grain_size_max = cat(grain_size_max, dim=0)
            fg_pair_idxs = cat(fg_pair_idxs, dim=0)
            if fg_rel_triplets.shape[0] > self.num_pos_per_img:
                perm = torch.randperm(fg_rel_triplets.shape[0], device=device)[:self.num_pos_per_img]
                fg_rel_triplets = fg_rel_triplets[perm]
                ori_rel_triplets = ori_rel_triplets[perm]
                grain_size_max = grain_size_max[perm]
        ...
        grain_size_labels = torch.zeros((bg_rel_inds.shape[0], 1), dtype=torch.float32, device=device)
        bg_rel_labs[:, 0] = 1.
        ...
        if fg_rel_triplets.shape[0] == 0 and bg_rel_triplets.shape[0] == 0:
            ...
            grain_size_labels = torch.zeros((1, 1), dtype=torch.float32, device=device)
            bg_rel_triplets[:, 0] = 1.
            bg_pair_idxs = torch.zeros((1, 2), dtype=torch.int32, device=device)
        return cat((fg_rel_triplets, bg_rel_triplets), dim=0), cat((fg_pair_idxs, bg_pair_idxs), dim=0), binary_rel, cat((ori_rel_triplets, bg_rel_triplets)), cat((grain_size_max, grain_size_labels))
...
def make_roi_relation_samp_processor(cfg):
    samp_processor = RelationSampling(
        ...
        cfg.MODEL.ROI_RELATION_HEAD.REQUIRE_BOX_OVERLAP, #F
        cfg.MODEL.ROI_RELATION_HEAD.NUM_SAMPLE_PER_GT_REL, #4
        cfg.MODEL.ROI_RELATION_HEAD.BATCH_SIZE_PER_IMAGE, #1024
        cfg.MODEL.ROI_RELATION_HEAD.POSITIVE_FRACTION, #0.25
        cfg.MODEL.ROI_RELATION_HEAD.USE_GT_BOX, #T
        cfg.TEST.RELATION.REQUIRE_OVERLAP, #F
    )
