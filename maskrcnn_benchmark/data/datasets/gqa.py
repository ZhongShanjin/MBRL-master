...
class GQADataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        #for GQA dataset
        # target, train_data = self.get_groundtruth(index, flip_img)
        ...
        #for GQA dataset
        # return img, target, index ,train_data
    ...
    def get_groundtruth(self, index, evaluation=False, flip_img=False):
        ...
        if evaluation:
            ...
            # train_data.extra_fields['width'] = self.img_info[index]['width']
            # train_data.extra_fields['height'] = self.img_info[index]['height']
            # train_data.extra_fields['img_path'] = os.path.join(self.img_dir, self.filenames[index])
            # train_data.add_field("boxes", target.bbox)
            # train_data.add_field("labels", target.extra_fields['labels'])
            # # for evaluation
            # nonzero_indices = torch.nonzero(target.extra_fields['relation'] > 0)
            # clip_relations = []
            # for row, col in nonzero_indices:
            #     value = target.extra_fields['relation'][row, col]
            #     clip_relations.append([row.item(), col.item(), value.item()])
            # clip_relations = np.array(clip_relations, dtype=np.int32)
            # train_data.add_field("relations", clip_relations.copy())
            # return target, train_data
            return target
