...
def do_vg_evaluation(
    ...
    if cfg.MODEL.ROI_RELATION_HEAD.NUM_CLASSES > 151:
...
def evaluate_relation_of_one_image(groundtruth, prediction, global_container, evaluator):
    ...
    local_container['pred_rel_labels'] = prediction.get_field('pred_rel_labels').detach().cpu().numpy()           # (#pred_objs, )
...
def oi_evaluate_relation_of_one_image(groundtruth, prediction, global_container, evaluator):
    """
    Returns:
        pred_to_gt: Matching from predicate to GT
        pred_5ples: the predicted (id0, id1, cls0, cls1, rel)
