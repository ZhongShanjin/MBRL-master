...
def compute_on_dataset(model, data_loader, device, need_confusion, synchronize_gather=True, timer=None):
    # 计算结果
    ...
    if need_confusion:
        confusion_matrix = pickle.load(open("inverse_confusion.pk", "rb"))
        ori_confusion_matrix = pickle.load(open("ori_inverse_confusion.pk", "rb"))
    for _, batch in enumerate(tqdm(data_loader)):
        with torch.no_grad(): # 使用model运算时, 不用计算梯度
            images, targets, image_ids = batch
            targets = [target.to(device) for target in targets]
            targets[0].add_field("confusion_matrix", confusion_matrix)
            targets[0].add_field("ori_confusion_matrix", ori_confusion_matrix)
            if timer:
                timer.tic()
            if cfg.TEST.BBOX_AUG.ENABLED:
                ...
                output = model(images.to(device), targets)  # 将计算结果转移到cpu上
            if timer:
                ...
                for oi in range(len(output)):
                    ...
                    for pari_idxs, pairs in enumerate(output[oi].extra_fields['rel_pair_idxs']):
                        ...
                        if r_name in confusion_matrix:
                            need_matrix += confusion_matrix[r_name]
                            flag_matrix = True
                        if r_name in ori_confusion_matrix:
                            need_matrix += ori_confusion_matrix[r_name]
                            flag_matrix = True
                        if flag_matrix:
...
def inference(
    ...
    if load_prediction_from_cache:
        ...
        predictions = compute_on_dataset(model, data_loader, device, cfg.Confusion, synchronize_gather=cfg.TEST.RELATION.SYNC_GATHER, timer=inference_timer)
