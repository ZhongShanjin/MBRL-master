...
path = "./hlm/motif_gqa_confusion.pk"

vocab = json.load(open("./datasets/gqa/GQA_200_ID_Info.json", "r"))
idx2lb = {int(k): v for k, v in vocab["obj_id_to_name"].items()}
lb2idx = {k: int(v) for k, v in vocab["obj_name_to_id"].items()}
idx2pred = {int(k): v for k, v in vocab["rel_id_to_name"].items()}
pred2idx = {k: int(v) for k, v in vocab["rel_name_to_id"].items()}

l = pickle.load(open(path, "rb"))

for idx, cur_data in tqdm(enumerate(l)):
    num_prp = cur_data['boxes'].shape[0]
    if len(cur_data["relations"]) != 0:
        pairs = cur_data["relations"][:, :2]
        if num_prp <= pairs.max():
            print("index error")

for idx, _ in tqdm(enumerate(l)):
    l[idx]['ori_relations'] = deepcopy(l[idx]['relations'])
    l[idx]['grain_size'] = np.ones( len(l[idx]['relations']) )
...
for i, data in tqdm(enumerate(l)):
    ...
    threshold_value = len(data['labels'])
    if len(relation_tuple) != 0:
        rows_to_delete = np.any(relation_tuple[:,:2] >= threshold_value, axis=1)
        if np.any(rows_to_delete):
            relation_tuple = relation_tuple[~rows_to_delete]
    else:
        continue
    sub_idxs, obj_idxs, rels = relation_tuple[:, 0], relation_tuple[:, 1], relation_tuple[:, 2]
    if (len(obj_idxs)==0):
        continue
    elif (labels.shape[0] <= obj_idxs.max() or labels.shape[0] <= sub_idxs.max()):
        print('index error')
...
def vis_triplet(triplet):
    ...
    prds = [idx2pred.get(int(i+1), "none") for i in idxs]
...
def collect_all_parent_data(query_parents, query_pair, son_rel_idx):
    ...
    logits = all_triplet_logits[flag, son_rel_idx-1]
...
for triplet in tqdm(all_triplets):
    ...
    idxs = idxs[mask]
    # Changing the granularity of abstract predicates

    for index in idxs:
        # if all_triplet_logits[index][you_rel] >= you_avg_logit:
        i, j = all_triplet_idxs[index]
        l[i]["relations"][j, 2] = pred2idx[r]
        if pred2idx[r] == 0 or pred2idx[r] == 100:
            continue
        all_changes[index] = importance_dic[triplet]
        l[i]['grain_size'][j] = importance_dic[triplet] / (
                    importance_dic.get((idx2pred[all_triplet_rels[index]], triplet[1], triplet[2]), 0) + importance_dic[
                triplet])
        # if l[i]['grain_size'][j] < 0.5:
            # print(l[i]['grain_size'][j])
        if l[i]["relations"][j, 2] == 0:
            print(l[i]['grain_size'][j])
    # modify relations
...
pickle.dump(l, open("motifs-gqa.pk_HLM_"+str(round(topk_percent, 3)), "wb"))
